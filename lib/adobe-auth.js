...
/**
 * Resolves the authentication provider based on the provided parameters.
 * Note that  IMS authentication options is preferred over Commerce integration options .
 * @param {object} params action input parameters.
 * @returns {ImsAuthProvider|IntegrationAuthProvider} returns the resolved authentication options
 * @throws {Error} if neither Commerce integration options nor IMS options are provided as params
 * @typedef {import('@adobe/aio-commerce-sdk/auth').ImsAuthProvider} ImsAuthProvider
 * @typedef {import('@adobe/aio-commerce-sdk/auth').IntegrationAuthProvider} IntegrationAuthProvider
 */
export async function resolveAuthOptions(params) {
  if (
    ...
    const integrationAuthParams = {
      consumerKey: params.COMMERCE_CONSUMER_KEY,
      consumerSecret: params.COMMERCE_CONSUMER_SECRET,
      accessToken: params.COMMERCE_ACCESS_TOKEN,
      accessTokenSecret: params.COMMERCE_ACCESS_TOKEN_SECRET,
    };
    assertIntegrationAuthParams(integrationAuthParams);
    return getIntegrationAuthProvider(integrationAuthParams);
  }
