...
export async function getAdobeAccessToken(params) {
  const config = {
    ...
    ims_org_id: params.AIO_COMMERCE_AUTH_IMS_IMS_ORG_ID,
...
export async function resolveAuthOptions(params) {
  ...
  throw new Error(
    "Can't resolve authentication options for the given params. " +
      'Please provide either IMS options (AIO_COMMERCE_AUTH_IMS_CLIENT_ID, AIO_COMMERCE_AUTH_IMS_CLIENT_SECRETS, AIO_COMMERCE_AUTH_IMS_TECHNICAL_ACCOUNT_ID, AIO_COMMERCE_AUTH_IMS_TECHNICAL_ACCOUNT_EMAIL, AIO_COMMERCE_AUTH_IMS_IMS_ORG_ID, AIO_COMMERCE_AUTH_IMS_SCOPES) ' +
      'or Commerce integration options (AIO_COMMERCE_AUTH_INTEGRATION_CONSUMER_KEY, AIO_COMMERCE_AUTH_INTEGRATION_CONSUMER_SECRET, COMMERCE_ACCESS_TOKEN, COMMERCE_ACCESS_TOKEN_SECRET). ' +
