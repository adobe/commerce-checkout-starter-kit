...
export async function main(params) {
  const logger = AioLogger("set-configuration", { level: params.LOG_LEVEL || "info" });
  try {
    const lib = init();
    let body;
    ...
    logger.debug(`Setting configuration with params: ${inspect({ id, code, level })}`);

    if (!(id || (code && level))) {
      logger.warn("Invalid params: Either id or both code and level query params are required");
      return badRequest({ 
        body: {
          code: 'INVALID_PARAMS',
    ...
    if (!(candidateConfig && Array.isArray(candidateConfig))) {
      logger.warn("Invalid body: request must include a config array in params.config or body.config");
      return badRequest({ 
        body: {
          code: 'INVALID_BODY',
    ...
    logger.error(`Something went wrong while setting configuration: ${inspect(error)}`);
