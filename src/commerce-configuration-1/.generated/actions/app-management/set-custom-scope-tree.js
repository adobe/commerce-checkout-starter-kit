...
export async function main(params) {
  const logger = AioLogger("set-custom-scope-tree", { level: params.LOG_LEVEL || "info" });
  try {
    const scopes = params.scopes;
    logger.debug(`Setting custom scope tree with ${scopes?.length || 0} scopes`);
    ...
    logger.error(`Something went wrong while updating custom scope tree: ${inspect(error)}`);
