...
export async function main(params) {
  const logger = AioLogger("get-config-schema", { level: params.LOG_LEVEL || "info" });
  try {
    const config = init();
    ...
    logger.debug(`Successfully retrieved configSchema: ${inspect(configSchema)}`);
    return ok({
      body: {
        configSchema
      }
    ...
    logger.error(`Something wen't wrong while retrieving config schema: ${inspect(error)}`);
