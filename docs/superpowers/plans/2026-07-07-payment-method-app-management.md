...
- Directory is `apps/payment-method/` — **not** top-level `payment-method/`. (Superseded: see "Corrections from the built apps" below — `shipping-method/` was itself relocated under `apps/` after the design spec was written, and `totals-collector/` followed the same convention.)
...
- `src/commerce-extensibility-1/ext.config.yaml` — the real runtime manifest. Its `hooks.pre-app-build`, `operations.workerProcess`, and an `app-management` action package are **auto-generated** ("do not remove or manually edit" — 3 actions: `app-config`, `association`, `installation`). The domain's own action packages (`commerce-checkout-starter-kit` for `info`, `payment-method` for `validate-payment`/`filter-payment`) are hand-appended below that in the same file.
...
- Actions live at `src/commerce-extensibility-1/actions/<name>/index.js`, not flat `actions/<name>/index.js`. A shared `lib/` (if needed — payment's `validate-payment`+`filter-payment` warrant one per the design spec's own file-layout rule) goes at `src/commerce-extensibility-1/lib/`, sibling to `actions/`.
- Replace Task 1's `app.config.yaml` content with this structure; add `install.yaml` and `src/commerce-extensibility-1/ext.config.yaml` as new files.

**3. The auth-swap decision (Task 9) was answered — but not the axis this plan gated on.** The design spec's flagged risk was about the *outbound* Commerce-client axis (`getCommerceClient` vs. the legacy OAuth1/IMS client for actions that call Commerce) — correctly identified as N/A here since neither `validate-payment` nor `filter-payment` calls Commerce. But `shipping-method/`'s real `shipping-methods` action revealed a **separate, previously-undocumented axis**: the *inbound* webhook-authentication mechanism itself. The built action has **no `webhookVerify`, no `COMMERCE_WEBHOOKS_PUBLIC_KEY`, no `raw-http: true`**. It runs with `require-adobe-auth: true` and relies entirely on a declarative `webhooks[]` entry in `app.commerce.config.ts` (field `requireAdobeAuth: true`) — the App Management platform verifies Commerce's webhook signature itself and invokes the action as a normal authenticated Runtime call. This axis applies regardless of whether the action calls Commerce outbound, so it **does** apply to `validate-payment`/`filter-payment`. Concretely:
   - Task 3 (`lib/webhook.js` / `webhookVerify`) should be **dropped entirely** — there is no signature-verification code left to extract.
   - Task 6 (`app.config.yaml`, now `ext.config.yaml` per correction 2): both actions get `require-adobe-auth: true`, no `raw-http` annotation, no `COMMERCE_WEBHOOKS_PUBLIC_KEY` input.
   - Task 4 (relocating the actions): drop the `webhookVerify` call and its failure branch, and drop `JSON.parse(atob(params.__ow_body))` — Commerce's webhook fields arrive already parsed as top-level keys on `params`. Confirmed field names (read directly from the current `actions/validate-payment/index.js` and `actions/filter-payment/index.js`, matching `developer.adobe.com`'s payment-use-cases doc):
     - `validate-payment`: `params.payment_method`, `params.payment_additional_information`.
     - `filter-payment`: `params.payload` (containing `.cart`, `.customer`).
   - Task 9's `docs/AUTH_DECISION.md` should be rewritten to record this as **confirmed**, not conditional: inbound auth is `require-adobe-auth: true` + declarative `webhooks[]` (done, no gate), while outbound Commerce-client choice remains N/A/deferred (still correctly gated on `shipping-method/`'s spike, should a future change add a Commerce call).
   - Task 10's README "Webhook signature setup" section should be dropped (no `COMMERCE_WEBHOOKS_PUBLIC_KEY` to configure), and "Runtime auth strategy" simplified to note only the outbound axis remains open.
   - `env.dist` (Task 1) no longer needs `COMMERCE_WEBHOOKS_PUBLIC_KEY` — this app has no env vars of its own unless a future Commerce-client swap adds `AIO_COMMERCE_AUTH_IMS_*` inputs.

**4. `webhooks[]` schema in `app.commerce.config.ts` (Task 7) has fields this plan's version is missing.** Confirmed against Adobe's App Management webhooks docs and the real `apps/shipping-method/app.commerce.config.ts`: each entry needs `category` (one of `"validation"` | `"append"` | `"modification"`, ascending conflict priority — `"validation"` returns only success/validation ops, `"append"` only adds, `"modification"` uses add/replace/remove) and `requireAdobeAuth: true` at the entry level (must match the action's `require-adobe-auth` setting — see correction 3). `webhook.fields` (array of `{ name }`) is also a real, documented field for declaring which Commerce sends — check `payment-use-cases`' webhooks.xml examples for whether `validate-payment`/`filter-payment` need it (their raw-http `<field>` mappings, if any, are the source of truth). Best-guess `category` (verify before shipping): `"validation"` for `validate-payment` (only ever returns success/exception, never mutates Commerce data) and `"modification"` for `filter-payment` (removes entries from the available-methods list).

**5. Tooling: drop husky/lint-staged/TypeScript/tsconfig.** The real `apps/shipping-method/package.json` has no `husky`, `lint-staged`, `typescript`, or `@types/node` — scripts are just `test` (`vitest run --coverage`), `code:check`, `code:fix`, `postinstall`. `biome.jsonc` sets `"root": false` (temporary — repo-root `biome.jsonc` still exists until the monolith-removal PR) and excludes `.generated`. `vitest.config.js` uses `environment: "node", globals: true` (no `vitest.setup.js`) with `coverage.thresholds` — expect to tune `branches` below 70% if the domain's actions have dead defensive branches or catch blocks that can't be triggered by crafted input alone (this happened in `totals-collector/`; don't fight the number, adjust the threshold with a comment explaining why).

**6. `commerce-checkout-starter-kit/info` (Task 2) returns `ok()` from `core/responses`, not `{ statusCode: HTTP_OK }`.** The real action is `import { ok } from "@adobe/aio-commerce-sdk/core/responses"; export function main(_params) { return ok(); }` — confirmed via its test asserting `{ statusCode: 200, type: "success" }`. This is a materially different shape than `{ statusCode: HTTP_OK }` but was accepted as an equivalent "tracking behavior unchanged" interpretation (still 2xx, still no body). Drop the local `lib/http.js`/`HTTP_OK` import entirely for this action.

**7. `resolveImsAuthParams` comes from `@adobe/aio-commerce-sdk/auth`, not `@adobe/aio-commerce-lib-auth`.** Confirmed against the real `create-shipping-carriers.js`. Only relevant here if/when the outbound Commerce-client swap (correction 3) actually happens.
