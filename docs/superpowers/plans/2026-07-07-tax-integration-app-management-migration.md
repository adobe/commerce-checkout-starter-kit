...
- Directory is `apps/tax-integration/` — **not** top-level `tax-integration/`. (Superseded: see "Corrections from the built apps" below — `shipping-method/` was itself relocated under `apps/` after this plan and the design spec were written, and `totals-collector/` followed the same convention.)
...
## Corrections from the built `shipping-method`/`totals-collector` apps (read before starting — supersedes conflicting statements above and in the tasks below)

`shipping-method/` and `totals-collector/` have since been built and merged (or are in review). Several things this plan assumed turned out to be wrong once real code and Adobe's own docs were checked. This section is the authoritative correction — apply it when executing the tasks below rather than the superseded text. It does not rewrite every task's code block; where a task's YAML/TS content needs to change, the delta is described here and the task should be adapted accordingly. (The Admin UI migration — Phase 5, Tasks 13-19 — is unaffected by any of this; these corrections are scoped to scaffolding, config, and the two webhook actions.)

**1. Directory: `apps/tax-integration/`, not top-level `tax-integration/`.** `shipping-method/` was originally built top-level per the design spec, then relocated under `apps/` (see its git history: `Subfolder into apps`). `totals-collector/` followed the same convention from the start. Scaffold this plan's Task 1 output at `apps/tax-integration/`.

**2. Real scaffolding is generated by `@adobe/aio-commerce-lib-app`'s own CLI, not hand-authored.** The actual `apps/shipping-method/` layout is:
- `install.yaml` — declares the `commerce/extensibility/1` extension point.
- `app.config.yaml` — just an extension pointer: `extensions: { commerce/extensibility/1: { $include: src/commerce-extensibility-1/ext.config.yaml } }`. It is **not** the runtime-actions manifest.
- `src/commerce-extensibility-1/ext.config.yaml` — the real runtime manifest. Its `hooks.pre-app-build`, `operations.workerProcess`, and an `app-management` action package are **auto-generated** ("do not remove or manually edit" — 3 actions: `app-config`, `association`, `installation`). The domain's own action packages (`commerce-checkout-starter-kit` for `info`, `tax-integration` for `collect-taxes`/`collect-adjustment-taxes`) are hand-appended below that in the same file.
- `src/commerce-extensibility-1/.generated/` — compiled output (`app.commerce.config.js`, `app.commerce.manifest.json`, the 3 `app-management` action files) produced by `npm install`'s `postinstall` hook (`npx aio-commerce-lib-app hooks postinstall`). Do not hand-edit.
- `package.json` needs `"postinstall": "npx aio-commerce-lib-app hooks postinstall"` and `"imports": { "#app.commerce.config": "./src/commerce-extensibility-1/.generated/app.commerce.config.js" }`.
- The two webhook actions and `telemetry.js`/`checkout-metrics.js` (Task 4) live at `src/commerce-extensibility-1/actions/`, not flat `actions/`. `lib/webhook.js` (if it survives correction 3) goes at `src/commerce-extensibility-1/lib/`, sibling to `actions/`.
- This affects Task 1, 2, 8: replace the flat `app.config.yaml` runtime-manifest content with this structure; add `install.yaml` and `src/commerce-extensibility-1/ext.config.yaml` as new files. **The Admin UI extension (Phase 5) is a second, separate extension point** (`commerce/backend-ui/2`) alongside `commerce/extensibility/1` — both are declared in the same top-level `install.yaml`/`app.config.yaml`, each `$include`-ing its own `src/<extension-dir>/ext.config.yaml`.

**3. The auth-swap decision (Task 20) was answered — but not the axis this plan gated on.** The design spec's flagged risk was about the *outbound* Commerce-client axis (`getCommerceClient` vs. the legacy OAuth1/IMS client for actions that call Commerce) — correctly identified as N/A here since neither `collect-taxes` nor `collect-adjustment-taxes` calls Commerce. But `shipping-method/`'s real `shipping-methods` action revealed a **separate, previously-undocumented axis**: the *inbound* webhook-authentication mechanism itself. The built action has **no `webhookVerify`, no `COMMERCE_WEBHOOKS_PUBLIC_KEY`, no `raw-http: true`**. It runs with `require-adobe-auth: true` and relies entirely on a declarative `webhooks[]` entry in `app.commerce.config.ts` (field `requireAdobeAuth: true`) — the App Management platform verifies Commerce's webhook signature itself and invokes the action as a normal authenticated Runtime call. This axis applies regardless of whether the action calls Commerce outbound, so it **does** apply to `collect-taxes`/`collect-adjustment-taxes`. Concretely:
   - Task 3 (`lib/webhook.js` / `webhookVerify`) should be **dropped entirely** — there is no signature-verification code left to extract.
   - Task 8 (`app.config.yaml`, now `ext.config.yaml` per correction 2): both actions get `require-adobe-auth: true`, no `raw-http` annotation, no `COMMERCE_WEBHOOKS_PUBLIC_KEY` input.
   - Tasks 5 and 6 (relocating the actions): drop the `webhookVerify` call and its failure branch, and drop `JSON.parse(atob(params.__ow_body))` — Commerce's webhook fields arrive already parsed as top-level keys on `params`. Confirmed field names (read directly from the current `actions/collect-taxes/index.js` and `actions/collect-adjustment-taxes/index.js`):
     - `collect-taxes`: `params.oopQuote` (e.g. `params.oopQuote.items`).
     - `collect-adjustment-taxes`: `params.oopCreditMemo` (e.g. `params.oopCreditMemo.items`, `params.oopCreditMemo.adjustment`). Note: an earlier pass at this correction summarized a public docs page as if `order_id`/`adjustment`/`items`/etc. were top-level fields — they are **not**; those are nested inside `oopCreditMemo`. Trust the current action code over any docs summary if they conflict.
   - Task 20's regression test/decision record should be rewritten to record this as **confirmed**, not conditional: inbound auth is `require-adobe-auth: true` + declarative `webhooks[]` (done, no gate), while outbound Commerce-client choice remains N/A/deferred (still correctly gated on `shipping-method/`'s spike, should a future change add a Commerce call).
   - Task 23's README "Configure webhook signature verification" section should be dropped (no `COMMERCE_WEBHOOKS_PUBLIC_KEY` to configure), and "Auth strategy for collect-taxes/collect-adjustment-taxes" simplified to note only the outbound axis remains open.
   - `env.dist` (Task 1) no longer needs `COMMERCE_WEBHOOKS_PUBLIC_KEY`.

**4. `webhooks[]` schema in `app.commerce.config.ts` (Task 21) has fields this plan's version is likely missing.** Confirmed against Adobe's App Management webhooks docs and the real `apps/shipping-method/app.commerce.config.ts`: each entry needs `category` (one of `"validation"` | `"append"` | `"modification"`, ascending conflict priority — `"validation"` returns only success/validation ops, `"append"` only adds, `"modification"` uses add/replace/remove) and `requireAdobeAuth: true` at the entry level (must match the action's `require-adobe-auth` setting — see correction 3). `webhook.fields` (array of `{ name }`) is also a real, documented field for declaring which fields Commerce should send — both `collect-taxes` and `collect-adjustment-taxes` almost certainly need this given how deeply nested `oopQuote`/`oopCreditMemo` are; check `tax-use-cases`' webhooks.xml examples for the exact `<field>` mappings before writing this task's config. Both actions calculate/replace tax amounts, so `category: "modification"` for both is the likely correct value — verify before shipping.

**5. Tooling: drop husky/lint-staged/TypeScript/tsconfig.** The real `apps/shipping-method/package.json` has no `husky`, `lint-staged`, `typescript`, or `@types/node` — scripts are just `test` (`vitest run --coverage`), `code:check`, `code:fix`, `postinstall`. `biome.jsonc` sets `"root": false` (temporary — repo-root `biome.jsonc` still exists until the monolith-removal PR) and excludes `.generated`. `vitest.config.js` uses `environment: "node", globals: true` (no `vitest.setup.js`) with `coverage.thresholds` — expect to tune `branches` below 70% if the domain's actions have dead defensive branches or catch blocks that can't be triggered by crafted input alone (this happened in `totals-collector/`; don't fight the number, adjust the threshold with a comment explaining why). Task 2's `vitest.setup.js` should likely be dropped for the same reason.

**6. `commerce-checkout-starter-kit/info` (Task 7) returns `ok()` from `core/responses`, not `{ statusCode: HTTP_OK }`.** The real action is `import { ok } from "@adobe/aio-commerce-sdk/core/responses"; export function main(_params) { return ok(); }` — confirmed via its test asserting `{ statusCode: 200, type: "success" }`. This is a materially different shape than `{ statusCode: HTTP_OK }` but was accepted as an equivalent "tracking behavior unchanged" interpretation (still 2xx, still no body). Drop the local `HTTP_OK`/`lib/http.js` import entirely for this action.

**7. `resolveImsAuthParams` comes from `@adobe/aio-commerce-sdk/auth`, not `@adobe/aio-commerce-lib-auth`.** Confirmed against the real `create-shipping-carriers.js`. Only relevant here if/when the outbound Commerce-client swap (correction 3) actually happens, or for `create-tax-integrations.js`'s rewrite (Task 12) if it calls `getCommerceClient` the same way `create-shipping-carriers.js` does.

**8. Beta packages require Adobe's internal Artifactory pre-release npm feed, not public npm.** `@adobe/aio-commerce-lib-app@1.8.0-beta-...`, `@adobe/aio-commerce-sdk@1.4.0-beta-...`, and `@adobe/aio-commerce-lib-admin-ui@0.2.0-beta-...` are not on `registry.npmjs.org`. `~/.npmrc` needs `@adobe:registry=https://artifactory-uw2.adobeitc.com/artifactory/api/npm/npm-aio-commerce-sdk-pre-release/` plus an `_authToken` pointing at a valid Artifactory API token env var. If `npm install` 401s, this is almost certainly why — check the registry config before assuming a dependency problem.
